...
Invoice.prototype = {
    ...
    calcTotal: function () {
         jQuery($.opt.parentClass).each(function (i) {
             var row = jQuery(this);
             var total = row.find($.opt.total).val();
         });
     ...
     */
    newRow: function () {
        jQuery(".item-row:last").after('<tr class="item-row"><td class="item-name"><div class="delete-btn"><input type="text" class="form-control item" placeholder="Service" type="text"><a class=' + $.opt.delete.substring(1) + ' id="delete" href="javascript:;" title="Remove row">X</a></div></td><td><input class="form-control price" placeholder="Description" type="text"> </td><td><input class="form-control qty" placeholder="Quantity" type="text"></td><td><input class="form-control total" value="0.00" type="text"></td></tr>');
